...
function sortEntries(entries: IGatherEntry[]) {
  ...
  document.querySelectorAll(`.${className}`).forEach(element => {
    const entry = entries.find(entry => entry.element === element);
    if (entry) {
...
export function useGatheredData<T>(match: (elem: any) => elem is T): T[];
export function useGatheredData<T>(match: (elem: any) => boolean): T[];
...
export function useGatheredElements<T>(match: (elem: any) => elem is T): Array<IGatheredElement<T>>;
export function useGatheredElements<T>(match: (elem: any) => boolean): Array<IGatheredElement<T>>;
